...
WITH cte AS (
  SELECT
    customer_id,
    plan_id,
    RANK() OVER(PARTITION BY customer_id ORDER BY start_date) subscription_order
  FROM
    foodie_fi.subscriptions
),
...
churn_count AS (
  ...
  FROM
    cte a
    INNER JOIN cte b
    ON a.subscription_order=b.subscription_order-1
 ...
 SELECT 
  ...
  ROUND((churn_count.churn_count * 100.0)/(total_customer_count.total_customer_count),0) percentage_churn_count
 FROM
 	churn_count, total_customer_count;
...
SELECT
  ...
  ROUND((plan_details.converted_customer)*100.0/(total_customer_count.total_customer_count), 2) conversion_percentage
FROM
	plan_details
  CROSS JOIN
...
current_plan AS (
  ...
  FROM
  	...
  	INNER JOIN
  	foodie_fi.subscriptions subscriptions
  	ON cte.active_plan_date=subscriptions.start_date
...
SELECT 
  ...
  ROUND(customer_plan_count.customer_count*100.0/total_customer.total_customer,2) percentage_count
FROM
	customer_plan_count
  CROSS JOIN
  total_customer
